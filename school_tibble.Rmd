...
```{r}
total_budget <-sapply(total_budget, as.numeric)
mean_reading_score <- sapply(mean_reading_score, as.numeric)
mean_math_score <- sapply(mean_math_score, as.numeric)
```
...
data2 %>% 
  ...
  summarize(Avg.Reading.Score=mean(reading_score), Avg.Math.Score=mean(math_score), Total.Students=n())
```
